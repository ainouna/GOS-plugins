...
class AutoMountManager(Screen):
	...
	def updateList(self):
		...
		self.list.append((_("Add new network mount point"),"add", _("Add a new NFS or CIFS mount point to your STB."), okpng ))
		self.list.append((_("Mountpoints management"),"view", _("View, edit or delete mountpoints on your STB."), okpng ))
		for file in listdir('/etc/enigma2'):
			if file.endswith('.cache'):
				if file == 'networkbrowser.cache':
					...
					break
		self.list.append((_("Change hostname"),"hostname", _("Change the hostname of your STB."), okpng))
	...
	def hostEdit(self):
		if os_path.exists("/etc/hostname"):
			...
			fp.close()
			self.session.openWithCallback(self.hostnameCallback, VirtualKeyBoard, title = (_("Enter new hostname for your STB")), text = self.hostname)
