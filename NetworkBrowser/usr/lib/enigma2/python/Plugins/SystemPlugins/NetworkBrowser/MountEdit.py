...
class AutoMountEdit(Screen, ConfigListScreen):
    ...
    def selectionChanged(self):
        ...
        if current == self.activeEntry or current == self.ipEntry or current == self.mounttypeEntry or current == self.hdd_replacementEntry or current == self.myBuffers:
            self["VKeyIcon"].hide()
            self["VirtualKB"].setEnabled(False)
        else:
    ...
    def updateConfig(self, ret = False):
        if (ret == True):
            ...
            iAutoMount.setMountsAttribute(self.sharenameConfigEntry.value, "options", self.optionsConfigEntry.value + self.CurrentBuffers.value)
    ...
    def updateConfigfinishedCB(self,data):
        if data is True:
            self.session.openWithCallback(self.Updatefinished, MessageBox, _("Your network mount has been updated.\nReboot tuner to reconfigure autofs."), type = MessageBox.TYPE_INFO, timeout = 10)
    ...
    def applyConfig(self, ret = False):
        if (ret == True):
            ...
            data['options'] =  self.optionsConfigEntry.value + self.CurrentBuffers.value
    ...
    def applyConfigfinishedCB(self,data):
        if data is True:
            self.session.openWithCallback(self.applyfinished, MessageBox, _("Your network mount has been activated.\nReboot tuner to reconfigure autofs."), type = MessageBox.TYPE_INFO, timeout = 10)
