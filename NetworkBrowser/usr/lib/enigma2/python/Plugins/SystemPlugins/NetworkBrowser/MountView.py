...
class AutoMountView(Screen):
        ...
        def delete(self, returnValue = None):
                ...
                if cur:
                        returnValue = cur[1]
                        self.applyConfigRef = self.session.openWithCallback(self.applyConfigfinishedCB, MessageBox, _("Please wait while removing your network mount..."), type = MessageBox.TYPE_INFO, timeout = 5)
                        iAutoMount.removeMount(returnValue,self.removeDataAvail)

        def removeDataAvail(self, data):
                print "[MountView.py] removeDataAvail"
                if data is True:
        ...
        def applyConfigfinishedCB(self,data):
                print "[MountView.py] applyConfigfinishedCB"
                if data is True:
                        self.session.openWithCallback(self.ConfigfinishedCB, MessageBox, _("Your network mount has been removed.\nReboot tuner to reconfigure autofs."), type = MessageBox.TYPE_INFO, timeout = 5)
