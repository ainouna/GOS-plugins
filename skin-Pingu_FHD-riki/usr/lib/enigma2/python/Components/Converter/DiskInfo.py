                ...
                for count in range(len(hddlist)):
                    ...
                    if hdd.mountDevice() == '/media/%s' % self.device:
                        if self.type is self.capacity:
                            ...
                            data = self.paramert_str.replace('Format:', '').replace('hdd', '').replace('usb', '').replace('%C', hdd.capacity()).replace('%M', hdd.model()).replace('%S', self.filesystem(hdd.mountDevice())).replace('%D', self.devpoint(hdd.mountDevice())).replace('%F', tmpdata)
