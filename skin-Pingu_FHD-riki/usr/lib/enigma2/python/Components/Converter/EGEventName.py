...
class EGEventName(Converter, object):
    NAME = 0
    SHORT_DESCRIPTION = 1
    ...
    def getText(self):
        ...
        if event is None:
            ...
            if self.type == self.NEXT_NAME or self.type == self.NEXT_NAME_NEXTWT or self.type == self.NEXT_NAME_NEXT or self.type == self.NEXT_DESCRIPTION or self.type == self.NEXT_NAMEWT:
                reference = self.source.service
                info = reference and self.source.info
                if info is not None:
                    eventNext = self.epgcache.lookupEvent(['IBDCTSERNX', (reference.toString(), 1, -1)])
                    eventNextNext = self.epgcache.lookupEvent(['IBDCT', (reference.toString(),
                      0,
                      -1,
                      -1)])
                    if self.type == self.NEXT_NAME:
                        ...
                        if len(eventNext[0]) > 4 and eventNext[0][4]:
                            ...
                            return ''
                    elif len(eventNext[0]) > 6 and eventNext[0][6]:
