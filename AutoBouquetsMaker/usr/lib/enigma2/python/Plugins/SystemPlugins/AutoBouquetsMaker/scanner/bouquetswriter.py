        ...
        for key in transponders.keys():
            ...
            for key2 in transponder["services"].keys():
                ...
                try:
                    ...
                    try:
                        print>>log, "[BouquetsWriter] UnicodeDecodeError, service name '%s' contains illegal characters." % service_name
                        lamedb.write("%s\n" % service["service_name"].decode('latin-1').encode("utf8"))
                    except UnicodeDecodeError:
                        lamedb.write("%s\n" % ("".join(i for i in service["service_name"] if ord(i)<128)))
